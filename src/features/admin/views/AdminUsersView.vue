<template>
  <div class="p-6 max-w-6xl mx-auto space-y-6">
    <div class="app-surface rounded-2xl p-6 space-y-4">
      <div class="flex items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
          <button
              class="text-sm opacity-70 hover:opacity-100 disabled:opacity-50"
          ...
          <button
              class="text-sm rounded-xl px-3 py-2 border border-white/10 hover:opacity-90 disabled:opacity-50"
              @click="openCreate()"
              :disabled="store.isLoading"
          >
      ...
      <div v-if="store.error" class="rounded-xl p-3 border border-red-500/30 text-red-200">
      ...
      <div v-else class="space-y-2">
        <div
            ...
            class="rounded-2xl p-6 text-center text-sm opacity-70 border border-white/10"
        >
          Brak danych
        </div>
        ...
        <div
            ...
            :key="u.id"
            class="app-surface rounded-xl px-4 py-3 border border-white/10 flex items-center justify-between gap-4"
        >
          <div class="min-w-0">
          ...
          <div class="flex items-center gap-2 shrink-0">
            <button
                class="text-xs rounded-xl px-3 py-2 border border-white/10 hover:opacity-90"
                @click="openEdit(u)"
            >
              {{ t("admin.users.edit") }}
            </button>
            ...
            <button
                class="text-xs rounded-xl px-3 py-2 border border-red-500/30 text-red-200 hover:opacity-90"
    ...
    <div v-if="createOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-lg app-surface rounded-2xl p-6 space-y-4 border border-white/10">
        <div class="flex items-center justify-between">
          <h2 class="text-lg font-semibold">{{ t("admin.users.create") }}</h2>
          <button class="text-sm opacity-70 hover:opacity-100" @click="closeCreate">✕</button>
        </div>
        ...
        <div class="space-y-3">
          ...
          <div v-if="modalError" class="rounded-xl p-3 border border-red-500/30 text-red-200">
            {{ t("common.error") }}: {{ modalError }}
          </div>
    ...
    <div v-if="editOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-lg app-surface rounded-2xl p-6 space-y-4 border border-white/10">
        <div class="flex items-center justify-between">
          ...
          <button class="text-sm opacity-70 hover:opacity-100" @click="closeEdit">✕</button>
        </div>
    ...
    <div v-if="deleteOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="relative w-full max-w-md app-surface rounded-2xl p-6 space-y-4 border border-white/10">
